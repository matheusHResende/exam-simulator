...
const CodeRunner = forwardRef<CodeRunnerRef, CodeRunnerProps>(({ testCases, problemIndex, storageKey, onResultsChange, onRunningStateChange }, ref) => {
  ...
  const [code, setCode] = useState<string>(() => loadFromStorage<string>(codeKey) ?? STARTER_CODE);
  const [results, setResults] = useState<TestResult[] | null>(() => loadFromStorage<TestResult[]>(resultsKey));
  ...
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [problemIndex, codeKey, resultsKey]);
  ...
  const handleRun = useCallback(async () => {
    ...
    for (const tc of testCases) {
      try {
        const { stdout } = await runPythonCode(pyodide, code, tc.input);
  ...
  return (
    <PanelGroup direction="vertical" className="h-full w-full bg-white">
      ...
      {/* Resize Handle */}
      <PanelResizeHandle className="h-2 bg-slate-100 border-y border-slate-200 hover:bg-violet-100/50 active:bg-violet-100 transition-colors cursor-row-resize flex items-center justify-center group shrink-0">
        <div className="w-10 h-0.5 bg-slate-300 group-hover:bg-violet-400 rounded-full transition-colors" />
      </PanelResizeHandle>

      {/* Bottom Panel: Test Results / Console */}
      <Panel defaultSize={40} minSize={20} className="flex flex-col bg-white overflow-hidden">
