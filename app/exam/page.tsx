...
import GabaritoList from '@/components/exam/GabaritoList';
import type { Question, AnswerMap, ExamResult } from '@/types/exam';

type Status = 'idle' | 'quiz' | 'result' | 'history' | 'review';

const STORAGE_KEY = 'exam_simulator_history';
...
export default function ExamPage() {
  ...
  const handleFileUpload = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (e) => {
      ...
      if (parsed.length > 0) {
        ...
        if (parsed.length < 30) setNumQuestions(parsed.length);
      } else {
        alert('Erro no formato do CSV.');
      }
    };
  ...
  const lineChartData = useMemo(() => {
    ...
    const points = data.map((d, i) => ({
      ...
      y: height - padding - (d.percentage * (height - 2 * padding)) / 100,
        ...
        <div className="max-w-4xl mx-auto">
          ...
          <div className="bg-white p-8 rounded-[2rem] shadow-sm border border-slate-200 mb-10 overflow-hidden">
            <h3 className="text-lg font-bold text-slate-800 mb-6 flex items-center">
              <TrendingUp className="w-5 h-5 mr-2 text-indigo-600" /> Tendência de
              Desempenho (Últimas 10 provas)
            </h3>
            {lineChartData ? (
                  ...
                  {[0, 25, 50, 75, 100].map((val) => {
                    ...
                    return (
                      <g key={val}>
                        ...
                        <text x="30" y={y + 4} fontSize="10" className="fill-slate-400 font-bold" textAnchor="end">
                          {val}%
                        </text>
                      </g>
                    );
                  ...
                  {lineChartData.points.map((p, i) => (
                    <g key={i} className="group cursor-help">
                      ...
                      <text
                        ...
                        fontWeight="900"
                        textAnchor="middle"
                        className="fill-indigo-700 opacity-0 group-hover:opacity-100 transition-opacity"
                      >
                        {p.percentage}%
                      </text>
                    </g>
              ...
              <div className="h-48 flex items-center justify-center bg-slate-50 rounded-2xl border border-dashed border-slate-200">
                <p className="text-slate-400 font-medium italic">
                  Realize pelo menos 2 simulados para ver o gráfico de tendência.
                </p>
              </div>
            )}
          ...
          <div className="space-y-4">
            <h3 className="text-lg font-bold text-slate-800 px-2">
              Histórico Detalhado (Clique para Revisar)
            </h3>
            {examHistory.map((test) => (
              <button
                key={test.id}
                onClick={() => { setSelectedExam(test); setStatus('review'); window.scrollTo({ top: 0, behavior: 'smooth' }); }}
                ...
                <div className="flex items-center">
                  ...
                  <div className="text-left">
                    <p className="font-bold text-slate-800 group-hover:text-indigo-600 transition-colors">
                      {test.score} de {test.total} acertos
                    </p>
                    <p className="text-xs text-slate-400 flex items-center">
                      <Calendar className="w-3 h-3 mr-1" /> {test.date}
                    </p>
                  </div>
                </div>
        ...
        <div className="max-w-4xl mx-auto">
          ...
          <div className="bg-indigo-600 text-white p-8 rounded-[2rem] shadow-lg mb-10 flex flex-col md:flex-row items-center justify-between">
            <div className="mb-4 md:mb-0">
              <p className="text-indigo-100 text-sm font-bold uppercase tracking-widest">
                Desempenho nesta prova
              </p>
              <h3 className="text-4xl font-black">
                {selectedExam.score} de {selectedExam.total} acertos
              </h3>
            </div>
    ...
    return (
      <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center p-6">
        <div className="bg-white p-8 md:p-12 rounded-3xl shadow-xl border border-slate-200 max-w-2xl w-full text-center">
          <Link
            href="/"
            className="inline-flex items-center text-slate-400 hover:text-indigo-600 text-sm font-bold mb-6 transition-colors"
          >
          ...
          <p className="text-slate-600 mb-10 text-lg leading-relaxed">
            Carregue seu banco de questões em CSV e pratique quantas vezes quiser,
            ...
            <div className="space-y-6">
              <div className="bg-green-50 border border-green-200 p-4 rounded-xl flex items-center justify-between">
                <div className="flex items-center text-green-700">
                  ...
                  <div className="text-left">
                    <p className="font-bold text-sm leading-none">{fileName}</p>
                    <p className="text-xs opacity-75">{allQuestions.length} questões</p>
                  </div>
                </div>
                <button
                  onClick={() => { setAllQuestions([]); setFileName(''); }}
              ...
              <button
                ...
                className="w-full py-5 bg-indigo-600 text-white rounded-2xl font-bold text-xl hover:bg-indigo-700 transform transition active:scale-95 shadow-xl shadow-indigo-200"
              >
          ...
          {examHistory.length > 0 && (
            <button
              onClick={() => setStatus('history')}
              className="mt-6 flex items-center justify-center w-full p-4 text-slate-500 hover:text-indigo-600 font-bold transition-colors"
            >
          ...
          <div className="flex justify-between items-end mb-8">
            <div>
              ...
              <h2 className="text-2xl font-bold text-slate-800">
                ...
                <span className="text-slate-400 font-normal">de {currentQuiz.length}</span>
              </h2>
            </div>
        ...
        <div className="max-w-4xl mx-auto">
          <div className="bg-white p-6 sm:p-10 md:p-16 rounded-[2rem] sm:rounded-[3rem] shadow-xl border border-slate-200 text-center mb-12 relative overflow-hidden">
            ...
            <h2 className="text-4xl font-black text-slate-900 mb-2">
              {passed ? 'Aprovação!' : 'Continue Praticando'}
            </h2>
            <p className="text-slate-500 text-xl mb-10">
              Resultado salvo no seu histórico pessoal.
            </p>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-12">
              <div className="bg-slate-50 p-6 rounded-3xl border border-slate-100">
                <p className="text-xs font-black text-slate-400 uppercase mb-1">Acertos</p>
                <p className="text-3xl font-black text-slate-800">{score}</p>
              </div>
              <div className="bg-slate-50 p-6 rounded-3xl border border-slate-100">
                <p className="text-xs font-black text-slate-400 uppercase mb-1">Erros</p>
                <p className="text-3xl font-black text-slate-800">{total - score}</p>
              </div>
              <div className="bg-slate-50 p-6 rounded-3xl border border-slate-100">
                <p className="text-xs font-black text-slate-400 uppercase mb-1">Média</p>
                <p className={`text-3xl font-black ${passed ? 'text-green-600' : 'text-amber-600'}`}>
                  {percentage}%
                </p>
              </div>
              <div className="bg-slate-50 p-6 rounded-3xl border border-slate-100">
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              <button
                onClick={generateNewQuiz}
                className="flex items-center justify-center px-10 py-5 bg-indigo-600 text-white rounded-2xl font-black text-lg hover:bg-indigo-700 transition-all shadow-xl shadow-indigo-200 transform hover:-translate-y-1"
              >
                <RotateCcw className="w-6 h-6 mr-3" /> Repetir Simulado
              </button>
              <button
                onClick={() => setStatus('history')}
                className="flex items-center justify-center px-10 py-5 bg-slate-100 text-slate-600 rounded-2xl font-black text-lg hover:bg-slate-200 transition-all"
              >
                <TrendingUp className="w-6 h-6 mr-3" /> Ver Evolução
              </button>
            </div>
          </div>
          <h3 className="text-2xl font-black text-slate-800 flex items-center mb-8 px-4">
            <CheckCircle2 className="w-8 h-8 mr-3 text-indigo-600" /> Gabarito Corrigido
          </h3>
